...
public class BookMarkController {
    ...
    @GetMapping("/bookMarkTopic")
    public ResponseEntity<?> fetchTopicForBookMark(Principal principal){
        User user = userService.findByStaffId(principal.getName());
        List<TopicDto> topicDtoList = bookMarkService.findBookMarkTopic(user,true);
    ...
    @GetMapping("/total")
    public ResponseEntity<?> totalListOfPost(Principal principal){
        User user = userService.findByStaffId(principal.getName());
        int total = bookMarkService.totalPosts(user);
