...
function setMinEndDate() {
    ...
    // Compare with the start date and set minEndDate accordingly
    if (!isNaN(startDate.getTime()) && startDate > today) {
        minEndDate = startDate;
    }

    minEndDate.setDate(minEndDate.getDate()); // Minimum end date is one day after
    ...
    const formattedMinEndDate = `${yyyy}-${mm}-${dd}`;
    ...
    if (new Date(endDateInput.value) < minEndDate) {
        endDateInput.value = formattedMinEndDate;
    }
}
