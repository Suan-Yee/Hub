...
public class PostRestController {
   ...
   public ResponseEntity<List<Post>> findByTopicName(@PathVariable("id")Long groupId){
    ...
    public ResponseEntity<?> updatePost(@RequestParam("exitId") Long id,
                                        ...
                                        @RequestParam(value = "removedImages", required = false) List<String> removeImages) throws IOException, ExecutionException, InterruptedException, ExecutionException {
        ...
        log.info("Image Urls {}",removeImages);
        ...
        if (removeImages != null && !removeImages.isEmpty()) {
            for (String imageUrl : removeImages) {
                imageService.deleteImageByUrl(imageUrl,exitContent.getId());
            }
        }
        ...
        if (files != null && !files.isEmpty()) {
            ...
            for (CompletableFuture<String> future : uploadFutures) {
                String imageUrl = future.get(); // This blocks until the upload completes
                Image image = new Image();
                image.setName(imageUrl);
                image.setContent(exitContent);
                images.add(image);
